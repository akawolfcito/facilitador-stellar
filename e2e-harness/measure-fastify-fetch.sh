...
RUNS="${1:-20}"
LOGDIR="$ROOT/artifacts/e2e/fastify-fetch-runs"
mkdir -p "$LOGDIR"
...
echo "measuring typescript/http/fastify + typescript/http/fetch over $RUNS runs"
echo
...
for i in $(seq 1 "$RUNS"); do
  ...
  (cd "$WORK/e2e" && STELLAR_BAZAAR_ROOT="$ROOT" pnpm exec tsx test.ts \
    ...
    --schemes=exact \
    --extensions=bazaar \
    --servers=typescript/http/fastify \
